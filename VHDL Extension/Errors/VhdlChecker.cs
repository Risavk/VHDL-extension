...
{///<summary>
 ...
 /// </remarks>
    class VhdlChecker
    {
        private readonly VhdlCheckerProvider _provider;
        ...
        private readonly List<VhdlCheckerTagger> _activeTaggers = new List<VhdlCheckerTagger>();

        internal readonly string FilePath;
        internal readonly VhdlErrorsFactory Factory;

        internal VhdlChecker(VhdlCheckerProvider provider, ITextView textView, ITextBuffer buffer)
        {
            _provider = provider;
            _buffer = buffer;
            ...
            this.Factory = new VhdlErrorsFactory(this, new VhdlErrorsSnapshot(this.FilePath, 0));

            //Programatically open the error window
            Helper.OpenErrorWindow();
        }

        internal void AddTagger(VhdlCheckerTagger tagger)
        {
        ...
        internal void RemoveTagger(VhdlCheckerTagger tagger)
        {
            ...
            if (_activeTaggers.Count == 0)
            {
                ...
                _buffer.Properties.RemoveProperty(typeof(Spelling.SpellChecker));
        ...
        private void OnBufferChange(object sender, TextContentChangedEventArgs e)
        {
            ...
            var oldVhdlErrors = this.Factory.CurrentSnapshot;
            var newVhdlErrors = new VhdlErrorsSnapshot(this.FilePath, oldVhdlErrors.VersionNumber + 1);

            // Copy all of the old errors to the new errors unless the error was affected by the text change
            foreach (var error in oldVhdlErrors.Errors)
            {
                Debug.Assert(error.NextIndex == -1);

                var newError = VhdlError.CloneAndTranslateTo(error, e.After);

                if (newError != null)
                {
                    Debug.Assert(newError.Span.Length == error.Span.Length);

                    error.NextIndex = newVhdlErrors.Errors.Count;
                    newVhdlErrors.Errors.Add(newError);
                }
            }

            this.UpdateSpellingErrors(newVhdlErrors);
        ...
        private void DoUpdate()
        {
            ...
            if ((!_isDisposed) && (_dirtySpans.Count > 0))
            {
                ...
                if (line.Length > 0)
                {
                    ...
                    var oldLineErrors = new List<VhdlError>();
                    foreach (var error in oldVhdlErrors.Errors)
                    {
                        Debug.Assert(error.NextIndex == -1);
                        ...
                        {
                            error.NextIndex = newVhdlErrors.Errors.Count;
                            newVhdlErrors.Errors.Add(VhdlError.Clone(error));   // We must clone the old error here.
                        }
                    }

                    int expectedErrorCount = newVhdlErrors.Errors.Count + oldLineErrors.Count;
                    bool anyNewErrors = false;

                    var classifications = _classifier.GetClassificationSpans(line.Extent);

                    var errors = VhdlStructerMapper.VhdlErrors.Where(e => true).ToList();
                    VhdlStructerMapper.VhdlErrors.Clear();
                    foreach (var vhdlError in errors)
                    {
                        var err = VhdlError.CloneAndTranslateTo(vhdlError, _currentSnapshot);
                        if (err != null)
                        {
                            VhdlStructerMapper.VhdlErrors.Add(err);
                        }
                    }
                    var vError = VhdlStructerMapper.VhdlErrors.FirstOrDefault(e => line.Extent.Contains(e.Span));

                    if (vError != null)
                    {
                        newVhdlErrors.Errors.Add(new VhdlError(vError.Span, vError.ErrorType));
                        anyNewErrors = true;
                    }
                    ...
                    {
                        if (classification.ClassificationType.IsOfType("VHDL"))
                        {
                            ...
                            while (index < text.Length)
                            {
                                ...
                                if (errorStart < 0)
                                {
                                    newSpellingErrors.Errors.Add(new SpellingError(line.Extent, new List<string> { "test 1", "test 2" }));
                                    break;
                                }
                    ...
                    */
                    ...
                    if (anyNewErrors || (newVhdlErrors.Errors.Count != expectedErrorCount))
                    {
                        this.UpdateSpellingErrors(newVhdlErrors);
                    }
                    else
        ...
        private void UpdateSpellingErrors(VhdlErrorsSnapshot spellingErrors)
        {
        ...
        internal VhdlErrorsSnapshot LastSpellingErrors { get; private set; }
    }
}