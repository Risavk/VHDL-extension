...
namespace VHDL_Extension.Errors.Spelling
{
    ///<summary>
        ...
        private void DoUpdate()
        {
            ...
            if ((!_isDisposed) && (_dirtySpans.Count > 0))
            {
                ...
                if (line.Length > 0)
                {
                    ...
                    var vError = VhdlStructerMapper.VhdlErrors.FirstOrDefault(e => e.Line == line);

                    if (vError != null)
                    {
                        newSpellingErrors.Errors.Add(new SpellingError(vError.Span, new[] { "test 1" }));
                    }
